...
export const Login = () => {
  const { login: signIn, loginWithGoogle, loading, error, clearError } = useAuthStore();
    ...
    <div className="min-h-[calc(100vh-4rem)] flex flex-col items-center justify-center px-4 py-8 sm:py-12 bg-slate-50 dark:bg-slate-950 transition-colors duration-200">
      <div className="w-full max-w-sm sm:max-w-md lg:max-w-lg">
        ...
        <div className="flex flex-col items-center mb-6 sm:mb-8">
          ...
          <h2 className="text-xl sm:text-2xl font-semibold text-slate-900 dark:text-white tracking-tight text-center">Sign in to your account</h2>
          <p className="text-slate-500 dark:text-slate-400 text-sm mt-1.5 text-center">Welcome back! Please enter your details.</p>
        </div>
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4 sm:space-y-5">
            ...
            <div>
              <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-1.5 mb-1.5">
                <label className="text-sm font-medium text-slate-755 dark:text-slate-300">Password</label>
                <Link to="/forgot-password" className="text-xs font-medium text-brand-600 dark:text-brand-400 hover:text-brand-700">
                  Forgot password?
                </Link>
              </div>
              <Input
