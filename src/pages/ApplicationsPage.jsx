...
export const ApplicationsPage = () => {
  ...
  const applicationJobs = jobs.filter((job) =>
    ['wishlist', 'applied'].includes(job.status?.toLowerCase())
  );
    ...
    <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-8 flex-grow w-full flex flex-col gap-6 transition-colors duration-200">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-slate-500 dark:text-slate-400 mt-1 text-sm">
            Manage your initial job applications and wishlist roles.
          </p>
        </div>
      ...
      {loading && jobs.length === 0 ? (
        ...
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-lg p-12 text-center text-slate-500 dark:text-slate-400 shadow-2xs flex flex-col items-center gap-2">
          ...
          <span className="font-semibold text-slate-700 dark:text-slate-300">No applications logged</span>
          <span className="text-xs text-slate-450 dark:text-slate-500 max-w-xs leading-normal">Wishlist and active applied roles will show up here. Use the "Add Application" button to log a job listing.</span>
        </div>
      ) : (
        <JobList 
