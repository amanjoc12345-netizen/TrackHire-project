    ...
    <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-8 flex-grow w-full flex flex-col gap-6 transition-colors duration-200">
      ...
      <div>
        ...
        <p className="text-slate-500 dark:text-slate-400 mt-1 text-sm">
          Review and manage your incoming offers and contract proposals.
        </p>
      </div>
      ...
      {loading && jobs.length === 0 ? (
        <div className="flex-grow flex items-center justify-center min-h-[300px]">
          <div className="flex flex-col items-center gap-3">
            ...
            <span className="text-xs text-slate-505 dark:text-slate-400 font-semibold">Loading offers...</span>
          </div>
        </div>
      ) : offerJobs.length === 0 ? (
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-lg p-12 text-center text-slate-500 dark:text-slate-400 shadow-2xs flex flex-col items-center gap-2">
          <Gift className="h-8 w-8 text-slate-300 dark:text-slate-700" />
          <span className="font-semibold text-slate-700 dark:text-slate-300">No active offers</span>
          <span className="text-xs text-slate-450 dark:text-slate-500 max-w-xs leading-normal">Applications updated to the "Offer" status will be listed in this workspace.</span>
        </div>
      ) : (
        <JobList 
          jobs={offerJobs} 
      ...
      {toast && (
        ...
        >
          {toast.type === 'success' ? (
            <div className="flex h-5 w-5 items-center justify-center rounded-full bg-emerald-550 text-white">
